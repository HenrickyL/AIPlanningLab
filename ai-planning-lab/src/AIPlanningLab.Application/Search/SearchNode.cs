...
public sealed class SearchNode
{
    ...
    private readonly Func<SearchNode, int> _heuristic;
    ...
    public static SearchNode CreateRoot(
        ...
        Func<SearchNode, IEnumerable<SearchNode>> expand,
        Func<SearchNode, bool> isGoal,
        Func<SearchNode, int>? heuristic = null)
        => new(initialState, parent: null, action: null, depth: 0, pathCost: 0, expand, isGoal, heuristic ?? (_ => 0));
    ...
    public SearchNode CreateChild(IState newState, IAction actionApplied)
        => new(newState, parent: this, action: actionApplied, depth: Depth +1, pathCost: PathCost + actionApplied.Cost, _expand, _isGoal, _heuristic);
    ...
    /// <summary>h(n) — delega para a closure de heurística injetada pelo Planner.</summary>
    public int Heuristic() => _heuristic(this);
